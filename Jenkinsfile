pipeline {
  ...
  stages {
   ...
   stage ('DAST - OWASP ZAP') {
		   ...
		   sh 'ssh -o  StrictHostKeyChecking=no ubuntu@65.0.176.6 "sudo docker run --rm -v /home/ubuntu:/zap/wrk/:rw -t owasp/zap2docker-stable zap-full-scan.py -t http://3.110.168.8 :8080/WebGoat -x zap_report || true" '
              //  sh 'ssh -o  StrictHostKeyChecking=no ubuntu@65.0.176.6 "sudo docker run --rm -v /home/apps:/zap/wrk/:rw -t owasp/zap2docker-stable zap-full-scan.py -t http://3.110.168.8:9999/WebGoat -x zap_report || true" '
